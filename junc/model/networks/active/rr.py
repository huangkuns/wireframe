...
def saveJunction(exp_id, iteration, dataset_name='test_all'):
    ...
    src_dir = join(home, 'ws/dataset/v1.1_pl/') if dataset_name=='test_all' else join(home, 'ws/dataset/junc_bench/BSDS500/test')
    files = [x[:-4] if x.endswith('.jpg') else x for x in imgnames]
    gts = loadNewGt(src_dir, files)
    
    #save_root_dir = '/var/www/html/'
    ...
    for th in  [0, 1, 2, 3, 4, 5, 6, 7, 8]:
        ...
        #preds = loadRawPrediction(files, root_dir, exp_id, iteration, th, phase=dataset_name, theta_thresh = 0.5)
...
def save_single_image(in_, pred, gt, img_idx, H):    
    ...
    newim = showJunctionPrediction(img, pred_junctions, pred_thetas, display_=True if DEBUG else False, thickness=2, color=(0, 255, 200), rand_color=False)
